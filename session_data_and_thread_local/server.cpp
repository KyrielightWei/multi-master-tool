...
DEFINE_int32(port, 60066, "TCP Port of this server");
...
public:
    ...
    void Echo(google::protobuf::RpcController* cntl_base,const example::EchoRequest* request,example::EchoResponse* response,google::protobuf::Closure* done) {
        ...
        pthread_t pid;
        if (pthread_create(&pid, NULL, process_thread, job) != 0) {
...
void AsyncJob::run() {
    ...
    cout<<"Received request from " << cntl->remote_side() << " to " << cntl->local_side()<< "-- message:" << request->message();//<<" ,time:"<<dt<<endl;
...
int main(int argc, char* argv[]) {
    ...
    string log_path="./lock_table_server.log";
